...
x <- get_brmap("City") %>% 
  left_join(Proagro_soja_mun_geral, c("City" = "Cod_Municipio"))
```
...
### Valor Total Médio por Área

O Valor Médio por Área dos contratos do Proagro é de 1656.69, porém temos 2 municípios com valores muito baixos: Aratiba(RS) e Constantina(RS).
...
Para estes dois municípios acredito que tenha havido algum tipo de erro. Em uma das safras Constantina teve uma área toal mais de 10x maior que os outros anos e em Aratiba 15x.

```{r erros_valor_medio_defl_area}
Proagro_soja_mun_safra[Proagro_soja_mun_safra$Nome_Município == "Constantina", c("SAFRA", "PROGRAMA","TP_SEGURO","num_seguros","NR_AREA_TOTAL", "VL_TOTAL_deflacionado","VL_TOTAL_Medio_Area_Defl")] %>%
arrange(SAFRA,PROGRAMA) %>% 
 kable(caption = "Seguros Constantina") %>% kable_styling(full_width = F)

Proagro_soja_mun_safra[Proagro_soja_mun_safra$Nome_Município == "Aratiba", c("SAFRA", "PROGRAMA","TP_SEGURO","num_seguros","NR_AREA_TOTAL", "VL_TOTAL_deflacionado","VL_TOTAL_Medio_Area_Defl")] %>%
  arrange(SAFRA,PROGRAMA) %>% 
...
#plot_map_mun(dados = Proagro_soja_mun_geral, fill_var = "tmap", mp = (max(Proagro_soja_mun_geral$VL_TOTAL_Medio_Area_Defl) - min(Proagro_soja_mun_geral$VL_TOTAL_Medio_Area_Defl))/2)
...
#plot_map_mun(dados = Proagro_soja_mun_geral, fill_var = "VL_TOTAL_Medio_Area_Defl", mp = (max(Proagro_soja_mun_geral$VL_TOTAL_Medio_Area_Defl) - min(Proagro_soja_mun_geral$VL_TOTAL_Medio_Area_Defl))/2)
```
