...
teste_na <- teste_na[teste_na$num_na < 2,]
...
dados_union <- dados_comparacao_municipio_meio %>% union_all(dados_comparacao_municipio_maior) 
#%>%   union_all(dados_comparacao_municipio_menor) 
#%>%   union_all(dados_comparacao_todos_municipios_soja)
...
# Compute CLARA
clara.res <- clara(test_drivers, 3, stand = T, samples = 5000, pamLike = TRUE, correct.d = F)
